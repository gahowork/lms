	...
	<nav class="panel-heading navbar navbar-default">
		<div class="container-fluid">
			<div class="navbar-header">
				<div class="navbar-brand"><?= __('Usergroups') ?></div>
			</div>
		...
		<table class="table table-hover">
			...
			<tbody>
				<?php foreach ($usergroups as $usergroup): ?>
					<tr>
						<td class="id"><?= $this->Number->format($usergroup->id) ?></td>
						<td class="created"><?= h($usergroup->created) ?></td>
						<td class="description"><?= h($usergroup->description) ?></td>
						<td class="actions">
							<?= $this->Html->link('<span class="glyphicon glyphicon-eye-open"></span>', ['action' => 'view', $usergroup->id], ['class'=>'btn btn-info', 'escape'=>false]) ?>
						</td>
					</tr>
				<?php endforeach; ?>
			</tbody>
