	...
	<nav class="panel-heading navbar navbar-default">
		<div class="container-fluid">
			<div class="navbar-header">
				<div class="navbar-brand"><?= __('Questiontypes') ?></div>
			</div>
		...
		<table class="table table-hover">
			...
			<tbody>
				<?php foreach ($questiontypes as $questiontype): ?>
					<tr>
						<td class="id"><?= $this->Number->format($questiontype->id) ?></td>
						<td class="created"><?= h($questiontype->created) ?></td>
						<td class="description"><?= h($questiontype->description) ?></td>
						<td class="actions">
							<?= $this->Html->link('<span class="glyphicon glyphicon-eye-open"></span>', ['action' => 'view', $questiontype->id], ['class'=>'btn btn-info', 'escape'=>false]) ?>
						</td>
					</tr>
				<?php endforeach; ?>
			</tbody>
